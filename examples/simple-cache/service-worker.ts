...
self.addEventListener('fetch', (event: FetchEvent) => {
	...
	if (isDev() || url.hostname !== hostname) {
		return event.respondWith(fetch(request))
	}

	// We use the stale-while-revalidate cache strategy here
	...
	return event.respondWith(async function () {
		...
		const fresh = fetch(request).then(resp => {
			...
			if (swr && cached && resp.status < 400)
				// TODO: compare the response with the cached one
				resp.clone().arrayBuffer().then(async (buf: ArrayBuffer) => {
					const client = await self.clients.get(event.clientId)
					client?.postMessage({type: 'REFRESH', path: url.pathname, buf}, [buf])
				})
